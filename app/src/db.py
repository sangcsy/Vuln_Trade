...
MOCK_COMMUNITY_POSTS = [
    ...
    (
        "user26",
        "퀴아는 오늘 변속이 부드럽네요",
        "차트가 저단에서 고단으로 넘어가는 느낌입니다. 문제는 제가 아직 안전벨트만 붙잡고 있다는 점입니다.",
        "2026-04-26 19:05:00",
    ),
    (
        "user46",
        "두화는 이름처럼 두 번 확인하고 들어가야겠습니다",
        "한 번 보고 매수하면 꼭 제 평단만 외롭게 남더라고요. 오늘은 확인 버튼 앞에서 심호흡 두 번 했습니다.",
        "2026-04-26 12:10:00",
    ),
    (
        "user67",
        "제이뱅크는 앱보다 제 손익이 더 자주 튕깁니다",
        "핀테크라 빠를 줄 알았는데 제 수익 전환만 로그인 대기 중입니다. 비밀번호는 분명 맞는 것 같은데요.",
        "2026-04-25 20:35:00",
    ),
    (
        "user83",
        "수진바이오 임상보다 제 인내심이 먼저 시험대입니다",
        "뉴스 하나에 심장이 반응하고 호가 하나에 손가락이 반응합니다. 바이오 투자는 체력전이네요.",
        "2026-04-25 08:50:00",
    ),
    (
        "user109",
        "은결홀딩스 이름은 든든한데 제 멘탈은 홀딩 실패입니다",
        "장투하려고 샀는데 5분마다 확인 중입니다. 이 정도면 장투가 아니라 장시간 새로고침입니다.",
        "2026-04-24 19:05:00",
    ),
    (
        "user124",
        "대한모터스 오늘 엔진 소리 괜찮네요",
        "시동은 걸린 것 같은데 제 계좌는 아직 주차장입니다. 출발 신호만 기다려봅니다.",
        "2026-04-24 09:55:00",
    ),
    (
        "user151",
        "한결바이오는 이름처럼 한결같이 어렵습니다",
        "오르면 왜 올랐는지 모르겠고 내리면 왜 내렸는지 더 모르겠습니다. 그래도 관심종목에서는 못 빼겠네요.",
        "2026-04-23 17:45:00",
    ),
    (
        "user166",
        "경석네트워크 연결 상태는 좋은데 제 수익은 끊겼습니다",
        "네트워크 종목이라 그런지 기대감은 잘 연결됩니다. 제 매도 타이밍만 계속 timeout입니다.",
        "2026-04-23 09:35:00",
    ),
    (
        "user183",
        "민성게임즈 차트가 보스전 같습니다",
        "패턴은 보이는 것 같은데 들어가면 항상 맞습니다. 오늘도 포션 대신 물타기 버튼을 찾고 있습니다.",
        "2026-04-22 16:20:00",
    ),
    (
        "user199",
        "승원토건은 천천히 쌓는 맛이 있네요",
        "벽돌 쌓듯 모아가려 했는데 제 조급함이 먼저 준공됐습니다. 그래도 기초공사는 튼튼해 보입니다.",
        "2026-04-22 11:15:00",
    ),
]
...
def _ensure_mock_posts(cursor):
    for username, title, content, created_at in MOCK_COMMUNITY_POSTS:
        cursor.execute("SELECT id FROM users WHERE username=%s", (username,))
        ...
        if existing:
            cursor.execute(
                """
                UPDATE posts
                SET user_id=%s, content=%s, created_at=%s, updated_at=%s
                WHERE id=%s
                """,
                (user["id"], content, created_at, created_at, existing["id"]),
            )
            continue
        cursor.execute(
            """
            ...
            VALUES (%s, %s, %s, %s, %s)
            """,
            (user["id"], title, content, created_at, created_at),
        )
...
MOCK_COMMUNITY_COMMENTS = [
    ...
    ("퀴아는 오늘 변속이 부드럽네요", "user31", "저는 늘 급출발했다가 급정거합니다. 운전 습관부터 고쳐야겠네요."),
    ("퀴아는 오늘 변속이 부드럽네요", "user88", "안전벨트 매고 분할 탑승하면 멀미가 조금 덜합니다."),
    ("두화는 이름처럼 두 번 확인하고 들어가야겠습니다", "user53", "저는 세 번 확인했는데도 제 평단이 먼저 들어가 있었습니다."),
    ("제이뱅크는 앱보다 제 손익이 더 자주 튕깁니다", "user117", "수익 전환 인증번호가 제 폰에는 안 오는 것 같습니다."),
    ("수진바이오 임상보다 제 인내심이 먼저 시험대입니다", "user64", "바이오는 멘탈 임상이 제일 어렵다는 말에 동의합니다."),
    ("수진바이오 임상보다 제 인내심이 먼저 시험대입니다", "user175", "결과 발표 전까지 제 심박수도 같이 상장된 느낌입니다."),
    ("은결홀딩스 이름은 든든한데 제 멘탈은 홀딩 실패입니다", "user41", "장투를 결심하고 3분 뒤에 차트를 켜는 게 국룰입니다."),
    ("대한모터스 오늘 엔진 소리 괜찮네요", "user145", "저도 탑승 대기 중인데 제 현금이 아직 면허를 못 땄습니다."),
    ("한결바이오는 이름처럼 한결같이 어렵습니다", "user69", "관심종목에서 빼면 꼭 오를 것 같아서 못 빼는 병이 있습니다."),
    ("경석네트워크 연결 상태는 좋은데 제 수익은 끊겼습니다", "user112", "제 매도 버튼도 가끔 패킷 손실이 나는 것 같습니다."),
    ("민성게임즈 차트가 보스전 같습니다", "user132", "저는 튜토리얼도 못 끝냈는데 난이도가 하드모드네요."),
    ("민성게임즈 차트가 보스전 같습니다", "user191", "패턴 파악했다고 생각한 순간 새 페이즈 들어갑니다."),
    ("승원토건은 천천히 쌓는 맛이 있네요", "user101", "기초공사 튼튼하면 좋죠. 제 멘탈도 철근 보강이 필요합니다."),
]
...
def initialize_runtime_schema(app):
    ...
    try:
        with db.cursor() as cursor:
            ...
            for post_id, title, content, created_at in posts_seed:
                cursor.execute(
                    """
                    UPDATE posts
                    SET title=%s, content=%s, created_at=%s, updated_at=%s
                    WHERE id=%s
                    """,
                    (title, content, created_at, created_at, post_id),
                )
